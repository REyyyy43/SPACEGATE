...
const schema = a.schema({
  Complex: a.model({
    ...
    towers: a.hasMany('Tower', 'complexId'), // Updated relationship
    floorsPerTower: a.integer().required(),
    unitsPerFloor: a.integer().required(),
    units: a.hasMany('Unit', 'complexId'), // Updated relationship
    events: a.hasMany('Event', 'complexId'), // Updated relationship
    users: a.hasMany('User', 'assignedComplexId'), // Updated relationship
    admins: a.hasMany('AdminComplexAssignment', 'complexId'), // Updated relationship name
  }).authorization(allow => [
  ...
  Unit: a.model({
    ...
    complexId: a.id(), // Updated field name
    towerId: a.id(), // Reference to the parent tower
    floorId: a.id(), // Reference to the parent floor
    complex: a.belongsTo('Complex', 'complexId'), // Updated relationship
  ...
  User: a.model({
    ...
    assignedComplexId: a.id(), // Updated field name
    assignedComplex: a.belongsTo('Complex', 'assignedComplexId'), // Updated relationship
    currentUnit: a.hasOne('Unit', 'currentTenantId'), // Remove .required() to make it optional
    assignedComplexes: a.hasMany('AdminComplexAssignment', 'userId'), // Updated relationship name
  ...
  Event: a.model({
    ...
    endDate: a.datetime(),
    complexId: a.id(), // Made optional by removing .required()
    complex: a.belongsTo('Complex', 'complexId'), // Updated relationship
  ...
  Message: a.model({
    ...
    createdAt: a.datetime().default('1970-01-01T00:00:00.000Z'),
    createdBy: a.id(), // Removed .required()
  ...
  Photo: a.model({
    tenantId: a.id().required(), // Multi-tenancy support
    rentalUnitId: a.id(), // Foreign key to RentalUnit
  ...
  Lease: a.model({
    tenantId: a.id().required(), // Multi-tenancy support
    residentId: a.id(), // Reference to the resident
    resident: a.belongsTo('User', 'residentId'), // Add relationship to User
    unitId: a.id(), // The unit being leased
  ...
  // AdminComplexAssignment Model
  AdminComplexAssignment: a.model({
    tenantId: a.id().required(), // Multi-tenancy support
    userId: a.id(), // Reference to the User (now optional)
    user: a.belongsTo('User', 'userId'), // Add relationship to User
    complexId: a.id(), // Updated field name (now optional)
    complex: a.belongsTo('Complex', 'complexId'), // Updated relationship
  }).authorization(allow => [
  ...
  Tower: a.model({
    ...
    units: a.hasMany('Unit', 'towerId'), // Add this line to pair with Unit.tower
  }).authorization(allow => [
  ...
  Floor: a.model({
    tenantId: a.id().required(), // Multi-tenancy support
    towerId: a.id(), // Reference to the parent tower (now optional)
  ...
  Tenant: a.model({
    ...
    phone: a.string(),
    billingPlan: a.enum(['basic', 'premium']),
    createdAt: a.datetime().default('1970-01-01T00:00:00.000Z'),
    updatedAt: a.datetime().default('1970-01-01T00:00:00.000Z'),
  }).authorization(allow => [
    allow.groups(['superUser', 'admin', 'providerSuperAdmin']), // Add necessary groups here
  ]),
});
